...
app.post('/send', (req, res) => {
  ...
  const mjmlOut = mjml2html(tmpl(body))
  if (mjmlOut.errors && mjmlOut.errors.length > 0) {
    res.status(400).json(mjmlOut)
    return
  }
  const html = mjmlOut.html
  const message = {
    from: 'Imperial Cinema <cinema@imperial.ac.uk>',
    to: 'Imperial Cinema <cinema-list@imperial.ac.uk>',
    subject: body.subject,
    text: html, // sorry :(
    html: html,
    replyTo: 'Imperial Cinema <cinema@imperial.ac.uk>',
  }
